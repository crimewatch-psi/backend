...
class SupabaseSessionStore extends EventEmitter {
  constructor(options = {}) {
    ...
    if (!supabaseUrl) {
      throw new Error('SUPABASE_URL environment variable is required for session store');
    }

    if (!supabaseKey) {
      throw new Error('SUPABASE_ROLE_KEY or SUPABASE_KEY environment variable is required for session store');
    }

    console.log('🔧 Initializing Supabase session store with:', {
      url: supabaseUrl,
      keyType: process.env.SUPABASE_ROLE_KEY ? 'ROLE_KEY' : 'KEY',
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST116' || error.code === '42P01') {
          // No session found or table doesn't exist
